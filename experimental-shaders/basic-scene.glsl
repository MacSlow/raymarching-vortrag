// build/raymarcher-gl basic-scene.glsl 640 360 30 data/noise-64x64.png
#version 420
...
float sdTerrainOrig (in vec3 p, in float height)
{
    float h = .5*texture (iChannel0, .2*p.xz).r;
    h += .25*texture (iChannel0, .4*p.xz).r;
    h += .125*texture (iChannel0, .8*p.xz).r;
    h += .0625*texture (iChannel0, 1.6*p.xz).r;
    h /= (.5 + .25 + .125 + .0625);
    h += height;
    return p.y - .25*h;
}
